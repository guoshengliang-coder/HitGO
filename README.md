...
- 大字报（HIG-50）：一条文案配多个背景各出一条滚动文字视频。素材来源可以是上传的视频 / 图片或空白（`kind: video | image | blank`），成片时长跟文案走（`trim.duration`），文字在安全区框内滚动（`layers[].scroll`），朗读与重点词自动高亮同样走百炼（`POST /api/tts`、`POST /api/highlight`，`HIGHLIGHT_MODEL` 默认 `qwen-plus`）。
