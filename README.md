...
- 画面文字本地化（HIG-38，`docs/CONTRACT.md` §1 `screen_text` / §6）：识别烧在画面上的文字（百炼视觉模型，抽帧 + 感知去重，费用按帧算所以重复帧先丢掉）→ 擦成「无字版」源片（缺省 `ERASE_PROVIDER=local`，ffmpeg `delogo`，不出网不要凭据；接了云厂商就换成无痕修复）→ 把译文写回原位置，生成的是**普通可编辑文字图层**，字号 / 颜色 / 对齐按画面估、可手动调。硬字幕不逐句识别：它的内容就是口播，译文和时间直接复用改语言的结果。会动 / 带透视 / 带动画的文字和字体还原不在范围内。
