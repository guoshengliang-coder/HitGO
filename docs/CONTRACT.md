...
左栏勾选多条视频后的批量转语言沿用同一个逐视频 `POST /api/videos/{id}/localize` 契约：
前端把勾选列表快照作为目标，共享目标语言、音色、术语与 BGM 选择，逐条提交并跟踪；每条自动识别源语言，
未就绪、无音轨或已有转语言任务的条目跳过。已启用自动套用时，对每条成功生成的首个语言版本分别写回各自的
`edit_spec`，不依赖编辑器当前显示的是哪条视频；若硬字幕仍需付费擦除，则列为待套用并保留逐条确认入口。
真实 TTS 提供方合成时会验证 WAV 和最终混音确实含有可听信号；两次合成仍无声或混音无声的版本进入 `failed`，不会把无声素材登记为 `ready` 或替换已有配音。仅供本地测试的 Fake TTS 固定生成静音，不执行信号校验。
...
- **拆分图层**（HIG-79）与拆分音轨（见下方 `audio` 规则）对称，同样是**纯前端操作，不引入新字段**：一条图层在剪后时刻 `p`
  ...
  **字幕类文字图层**（`origin = "localize" | "subtitle"`，或名字形如「字幕 N」/「韩语字幕 N」）拆分时**文字也一起切开**
  ...
  其它文字图层仍是两段留同样的全文。
- **译文字幕自动断句**（HIG-36）：一条译文太长时，前端在套用时把它拆成多条相邻的文字图层——**纯前端，
  ...
  预览和导出都包含图层结束时刻，故相邻字幕的交界留 1 ms，避免同一帧叠字。
...
- **复合片段（HIG-85，编辑器专用，worker 全部忽略）**：`layers[].group`、`audio.tracks[].group`、
  ...
  在时间线上默认折叠为一条虚拟「复合轨」（只影响编辑器显示，不改变渲染契约）；手动复合允许单个成员，
  点复合条选中整组，双击展开 / 收起成员，
  随后的移动、删除、复制按多选处理。展开后点任一成员仍选中整组（⌥ 点只选单个）。「一键复合」按画面
  自动编组（id 前缀 `cg_`，每次重算）：拼接主轨片段 / 非拼接视频的保留段 / 上层视频片段各为一个画面片段，有具体时段的
  图层与独立音轨归到与其重叠最长、且重叠不少于自身时长一半的画面片段，关联原声随其上层片段；自动组仍至少两个成员。
  ⌘G 手动组合（id 前缀 `grp_`，一键复合不改动）可选任意类型和数量，⇧⌘G 解除。粘贴出的副本另起新组；
  手动组删到只剩一个成员时仍保留复合片段，自动组删到只剩一个成员时解散。
  ...
  非拼接主轨片段需要手动复合或拖动重排时，编辑器把现有保留段与分割点转成引用原视频的 `sequence.clips`，
  每段获得稳定 id；旧删除区间的源画面也留在序列中并继续由 `trim.remove` 排除，保留恢复入口。拖放按插入点紧密重排，
  不留空档或重叠。原声随画面走，完全归属该段的定时图层与独立音轨跟随，跨段内容保留在原成片时间。
  转换前的旧 `trim.splits` 数据仍可读取；转换后不再发送 `splits`。
...
- **自动字幕标记 `auto`**（文字图层可选，缺省 false / 缺席，HIG-84）：`origin = "subtitle"` 且 `auto = true` 表示这一层是
  ...
  超出成片正片时长的截掉；相邻识别字幕同样在交界留 1 ms；只铺第一遍（`trim.duration` 循环补足的后几遍不重复铺字幕）；最多 1200 条。
