...
function measureLine(ctx: Ctx2D, line: string, spacingPx: number, native: boolean): number {
  const s = line || ' ';
  if (!spacingPx) return ctx.measureText(s).width;
  if (native) {
    // 原生实现会在每个字形（含最后一个）后加字距；扣掉末尾那份让对齐更准
    return Math.max(0, ctx.measureText(s).width - spacingPx);
  }
  const gs = glyphs(s);
  let w = 0;
  for (const g of gs) w += ctx.measureText(g).width;
  return w + spacingPx * Math.max(0, gs.length - 1);
}

function drawLine(ctx: Ctx2D, line: string, x: number, y: number, spacingPx: number, native: boolean, mode: 'fill' | 'stroke') {
  const s = line || ' ';
...
export function drawTextImage(text: string, style: TextStyle, H = TEXT_CANVAS.H): RenderedText {
  ...
  const boxW = Math.ceil(contentW + padPx * 2 + strokePx * 2);
  ...
  const lineX = (i: number) => {
    ...
    if (style.align === 'center') return left + (contentW - lw) / 2;
    if (style.align === 'right') return left + contentW - lw;
    return left;
  };
  ...
  if (style.background) {
    ...
    const r = Math.min(padPx, fontPx * 0.2);
    ...
    lines.forEach((line, i) => {
      if (strokePx > 0) drawLine(ctx, line, lineX(i), lineY(i), spacingPx, native, 'stroke');
      drawLine(ctx, line, lineX(i), lineY(i), spacingPx, native, 'fill');
    });
    setShadow(false);
  }
  ...
  lines.forEach((line, i) => {
    ...
    if (strokePx > 0) {
      ctx.strokeStyle = style.stroke_color;
      ctx.lineWidth = strokePx * 2;
      drawLine(ctx, line, x, y, spacingPx, native, 'stroke');
    }
    ctx.fillStyle = style.color;
    drawLine(ctx, line, x, y, spacingPx, native, 'fill');
  });
...
export async function renderTextImage(layer: TextLayer): Promise<RenderedText> {
  ...
  return drawTextImage(layer.text, layer.style);
}
...
export function textCacheKey(layer: TextLayer): string {
  return JSON.stringify([layer.text, layer.style]);
}
