  ...
  return (
    <>
      ...
      {picker && <Modal title="添加视频片段" onClose={() => setPicker(false)} width={510}>
        ...
        <div className="sequence-picker-list">{videos.filter((v) => (v.kind === 'image' || (v.kind ?? 'video') === 'video') && v.name.toLowerCase().includes(query.toLowerCase())).map((v) => <button key={v.id} className="sequence-pick" disabled={v.status !== 'ready'} onClick={() => add(v.id)}><span>{v.name}{v.kind === 'image' ? ' · 图片' : ''}</span><small>{v.status === 'ready' ? formatSeconds(v.duration) : v.status === 'preparing' ? '预处理中…' : '预处理失败'}</small></button>)}</div>
